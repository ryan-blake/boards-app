...
def sortable(column, title = nil)
  ...
  link_to column + ":" "#{direction}", params.merge(:sort => column, :direction => direction, :page => nil).permit!, remote: true
end

def sortable_cat(column, title = nil)
  title ||= column.titleize
  direction = (column == sort_column && sort_direction == "asc") ? "desc" : "asc"
  link_to column , params.merge(:sort => column, :direction => direction, :page => nil).permit!, remote: true
end
